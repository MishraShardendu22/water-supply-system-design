...
export default function AdminDashboardPage() {
  ...
  const busyStationsCount = stations.filter(
      ...
      <div className="space-y-6">
        ...
        <div className="bg-[#2C5745] text-white p-6 rounded-xl shadow border border-[#1e3d30] flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-emerald-100 mt-1 max-w-xl">
              Real-time monitoring of tanker distribution, priority scoring,
              driver familiarity matching, and filling station congestion.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <StatCard
            ...
            value={
              loading ? "..." : `${availableDriversCount} / ${drivers.length}`
            }
          ...
          <StatCard
            ...
            value={
              loading ? "..." : `${availableVehiclesCount} / ${vehicles.length}`
            }
          ...
          <StatCard
            ...
            value={
              loading ? "..." : `${busyStationsCount} / ${stations.length}`
            }
            ...
            {loading ? (
              ...
              <div className="space-y-3">
                {stations.map((st) => (
                  <div
                    key={st.id}
                    className="p-3 bg-[#f7f4d9] rounded-lg border border-[#dcd499]"
                  >
                    ...
                    <div className="w-full h-2 bg-gray-200 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${Math.min(100, (st.currentTruckCount / 10) * 100)}%`,
                        }}
                      />
                    </div>
                  </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className="text-xs text-[#857c4c]">
                    Prioritized by urgency algorithm
                  </p>
                </div>
                <Link
                  href="/requests"
              ...
              {loading ? (
                ...
                <div className="p-8 border border-dashed border-[#e2dab0] rounded-lg text-center text-xs text-[#857c4c]">
                  <p className="font-bold text-[#2E2910] text-sm mb-1">
                    No Active Requests
                  </p>
                  <p>
                    There are currently no water supply requests in the
                    database.
                  </p>
                  <Link
                    href="/requests"
                  ...
                  <table className="w-full text-left text-xs">
                    ...
                    <tbody className="divide-y divide-[#f2ebd4]">
                      {requests.slice(0, 5).map((req) => (
                        <tr
                          key={req.id}
                          className="hover:bg-[#f7f4d9]/60 transition-colors"
                        >
                          <td className="p-2.5 font-medium">
                            <span className="font-mono font-bold text-[#2E2910]">
                              #{req.id.slice(-6)}
                            </span>
                            <span className="block text-[10px] text-[#857c4c]">
                              {req.requestType}
                            </span>
                          </td>
                          <td className="p-2.5">
