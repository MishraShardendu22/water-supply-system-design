...
import { Driver, PhoneType } from '../../lib/types';
...
export default function DriversPage() {
  ...
  const handleCreateDriver = async (e: React.FormEvent) => {
    ...
    setCreating(true);
    const res = await driversApi.createDriver({ name, contactNumber, phoneType });
    setCreating(false);
    ...
    <AppShell>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h2 className="text-xl font-bold text-[#2E2910]">Tanker Driver Management</h2>
            <p className="text-xs text-[#857c4c]">
              Registered drivers, phone capabilities, ratings, and locality delivery experience
            </p>
          </div>
          <button
        ...
        <div className="card-surface overflow-hidden">
          {loading ? (
            <div className="py-12 text-center text-xs text-[#857c4c]">Loading drivers...</div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs">
                ...
                <tbody className="divide-y divide-[#f2ebd4]">
                  {drivers.map((d) => (
                    <tr key={d.id} className="hover:bg-[#f7f4d9]/70 transition-colors">
                      <td className="p-3 font-bold text-[#2E2910]">{d.name}</td>
                      <td className="p-3 font-mono text-[#58512b]">{d.contactNumber}</td>
                      <td className="p-3">
                      ...
                      <td className="p-3">
                        <Badge variant={d.status === 'Available' ? 'available' : 'busy'}>
                          {d.status}
                        </Badge>
                      </td>
                    </tr>
        ...
        <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Register Tanker Driver">
          <form onSubmit={handleCreateDriver} className="space-y-4 text-xs">
            <div>
              <label className="block font-bold text-[#2E2910] uppercase mb-1">Full Name</label>
              <input
                type="text"
                value={name}
            ...
            <div>
              <label className="block font-bold text-[#2E2910] uppercase mb-1">Contact Number</label>
              <input
                type="text"
                value={contactNumber}
            ...
            <div>
              <label className="block font-bold text-[#2E2910] uppercase mb-1">Device Phone Type</label>
              <select
                value={phoneType}
                onChange={(e) => setPhoneType(e.target.value as PhoneType)}
                className="w-full px-3 py-2 border rounded text-xs outline-none"
              >
                <option value="Basic">Basic Feature Phone (Landmark Guided)</option>
